...
# 处理好之后的数据文件。
INPUT_DATA = '/home/shenxj/tf-work/datasets/flower_processed_data.npy'
# 保存训练好的模型的路径。
TRAIN_FILE = 'train_dir/model'
# 谷歌提供的训练好的模型文件地址。因为GitHub无法保存大于100M的文件，所以
# 在运行时需要先自行从Google下载inception_v3.ckpt文件。
CKPT_FILE = '/home/shenxj/tf-work/datasets/inception_v3.ckpt'
...
def main():
    ...
    with tf.Session() as sess:
        ...
        load_fn(sess)
	saver = tf.train.import_meta_graph('train_dir/model-999.meta')
	saver.restore(tf.get_default_session(),'train_dir/model-999')
