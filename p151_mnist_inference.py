...
# 过程。在这个程序中将用到dropout方法，dropout可以进一步提成模型可靠性并防止过拟合，
# dropout过程只在训练时使用
def inference(input_ternsor, train, regualrizer):
    # 声明第一层卷积层的变量并实现前向传播。这个过程和6.3.1小节中介绍的一致。
    # 通过使用不同的命名空间来隔离不同层的变量，这可以让每一层中的变量命名只需要
    # 考虑在当前层的作用。而不需要担心重名的问题。和标准LeNet-5模型不大一样，这里
    # 定义的卷积层输入为28x28x1的原始MNIST图片像素。因为卷积层中使用了全0填充，
    # 所以输出为28x28x32的矩阵
    with tf.variable_scope('layer1-conv1'):
        conv1_weigths = tf.get_variable(
        ...
        conv1 = tf.nn.conv2d(
            input_ternsor, conv1_weigths, strides=[1, 1, 1, 1], padding='SAME'
        )
        relu1 = tf.nn.relu(tf.nn.bias_add(conv1, conv1_biases))

    # 实现第二层池化层的前向传播过程。这里选用最大池化层，池化层过滤器的边长为2,
    # 使用全0填充且移动的步长为2.这一层的输入是上一层的输出，也就是28x28x32
    # 的矩阵。输出为14x14x32的矩阵。
    ...
    # 声明第三层卷积层的变量并实现前向传播。这一层的输入为14x14x32的矩阵。
    # 输出为14x14x64的矩阵
    ...
    # 向量长度为3136,输出是一组长度为512的向量。这一层和之前在第5章中介绍的基本
    ...
    with tf.variable_scope('layer5-fc1'):
        ...
        # 只有全链接层的权重需要加入正则化.
        if regualrizer != None:
            tf.add_to_collection('losses', regualrizer(fc1_weights))
    ...
    with tf.variable_scope('layer6-fc2'):
        ...
        if regualrizer != None:
            tf.add_to_collection('losses', regualrizer(fc2_weights))
