...
# 个张量名称就是‘pool_3/_reshape:0’.在训练的模型是，可以通过tensor.name来获取张
# 量的名称.
...
MODEL_DIR = '/home/shenxj/tf-work/datasets/inception_dec_2015'

# 下载的谷歌训练好的Inception-v3模型文件名
MODEL_FILE = 'tensorflow_inception_graph.pb'

# 因为一个训练数据会被使用多次，所以可以将原始图形通过Inception-v3模型计算得到
# 的特征向量保存在文件中，免去重复的计算。下面的变量定义了这些文件的存放地址
CACHE_DIR = '/home/shenxj/tf-work/tmp/bottleneck'

# 图片数据文件夹。在这个文件夹中每一个子文件夹代表一个需要区分的类别，每个子文件夹中
# 存放了对应类别的图片
INPUT_DATA = '/home/shenxj/tf-work/datasets/flower_photos'
...
# testing_percentage和validation_percent参数指定了测试数据集和验证数据集的大小
def create_image_lists(testing_percentage, validation_percentage):
    # 得到的所有图片都存在result这个字典（dictionary）里。这个字典的key为类别的名
...
# image_dir参数给出了根目录。存放图片数据的根目录和存放图片特征向量的更目录地址不同。
...
# 这个寒红粟使用加载的训练好的Inception-v3模型处理一张图片，得到这个图片的特征向量
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
    ...
    # 经过卷积神经网络处理的结果是一个四维数组，需要将这个结果压缩成一个特征
    # 向量（一维数组）
...
def get_random_cached_bottlenecks(
        sees, n_calsses, image_lists, how_many, category,
        jpeg_data_tensor, bottleneck_tensor):
    bottlenecks = []
    ground_truths = []
    for _ in range(how_many):
        # 随机一个类别和图片的编号加入当前的训练数据。
        label_index = random.randrange(n_calsses)
        label_name = list(image_lists.keys())[label_index]
        image_index = random.randrange(65536)
        bottleneck = get_or_create_bottleneck(
            sees, image_lists, label_name, image_index, category,
            jpeg_data_tensor, bottleneck_tensor)
        ground_truth = np.zeros(n_calsses, dtype=np.float32)
...
def main(_):
    ...
    # 读取已经训练好的Inception-v3模型。谷歌训练好的模型保存在了GraphDef Protocol
    ...
    # 定义新的神经网络输入，这个输入就是新的图片经过Inception-v3模型前向传播到瓶颈层
    ...
    # 的图片抽象为了更加容易分类的特征向量，所以不需要再训练那么复杂的神经网络来完成
    # 这个新的分类任务
