...
REGULARIZATION_RATE = 0.0001    # 描述模型复杂程度的正则化在损失函数中的系数
...
# 这样方便在测试时使用滑动平均模型。
def inference(input_ternsor, avg_class, weights1, biases1,
              weights2, biases2):
    # 当没有提供滑动平均类时，直接使用参数当前的取值
    if avg_class == None:
        # 计算隐藏层的前向传播结果，这里使用ReLU激活函数
        layer1 = tf.nn.relu(tf.matmul(input_ternsor, weights1) + biases1)

        # 计算输出层的前向传播结果。因为再去计算损失函数时会一并计算softmax函数，
        # 所以这里不需要加入激活函数。而且不加入softmax不会影响预测结果。因为预测时
        # 使用的是不同类型对应节点输出值的相对大小，有没有softmax层对最后分类结果的
        # 计算没有影响。于是在计算整个神经网络的前向传播时可以不加入最后的sotfmax层
        ...
        # 然后再计算相应的神经网络前向传播结果。
        layer1 = tf.nn.relu(
            tf.matmul(input_ternsor, avg_class.average(weights1)) +
...
def train(mnist):
    ...
    # GraphKeys.TRAINABLE_VARIABLE中的元素。这个集合的元素就是所有设定没有指定
    # trainables=False的参数
    variable_averages_op = variable_averages.apply(
        tf.trainable_variables()
    )

    # 计算使用了滑动平均之后的前向传播结果。第4章中介绍过滑动平均不会改变变量本省的
    ...
    # 供的spare_softmax_cross_entropy_with_logits函数来计算交叉熵。当分类
    ...
    # 标准答案是一个长度为10的以为数组，而该函数需要提供的是一个正确答案的数字，所以需
    ...
    learning_rate = tf.train.exponential_decay(
        LEARNING_RATE_BASE,     # 基础的学习率，随着迭代的进行，更新变量时使用的
                                # 学习率在这个基础上递减
        global_step,            # 当前迭代的轮数
        mnist.train.num_examples / BATCH_SIZE, #过完所有的训练数据需要的迭代数
    ...
    # 校验使用了滑动平均模型的神经网络前向传播结果是否正确。tf.argmax(average_y,1)
    # 计算每一个样例的预测答案。其中average_y是一个batch_size * 10的二位数组，每一行
    ...
    # 这个运算首先将一个布尔型的数值转换为实数型然后计算平均值。这个平均值就是模型在这
    # 一组数据上的正确率
